...
import { OnJoinCheck } from './OnJoinCheck';
import { OnJoinInputChange } from './OnJoinInputChange';
import styles from '../../Component/Login.module.css';



const Joininfo = ( { joininput, setJoininput }) => {



    const joincheck = OnJoinCheck(joininput, setJoininput);

    const onJoinChange = OnJoinInputChange(joininput, setJoininput);


    return (
        <div>
            <p><input type='checkbox' onClick={joincheck}/>
            <span style={{fontWeight : 'bold', fontSize:'12px'}}>이용약관 및 개인정보수집 및 이용, 쇼핑 정보 수신(선택)에 동의합니다.</span>
            </p>
            <p style={{fontSize: '11px'}}>[필수] 이용약관 동의</p>
            <textarea className = {styles.textareas} value='제1조(목적)
                ...
                부 칙(시행일) 이 약관은 년 월 일부터 시행합니다. 부 칙(시행일) 이 약관은 년 월 일부터 시행합니다.' disabled />
                
            
            <p style={{fontSize:'11px'}}>이용 약관에 동의하십니까? 
            <input type='checkbox' value = {joininput.first} name='first' onChange={onJoinChange} checked={joininput.first === true ? true : false}/></p>
            <br />
            <p style={{fontSize:'11px'}}>[필수] 개인정보 수집 및 이용 동의</p>
            <textarea disabled className={styles.textareas}
            ...
            <p style={{fontSize:'11px'}}>개인정보 수집 및 이용에 동의하십니까? 
            <input type='checkbox' value={joininput.second} name='second' onChange={onJoinChange} checked={joininput.second === true ? true : false} /></p>
            <br/>
            <p style={{fontSize:'11px'}}>[선택] 쇼핑정보 수신 동의</p>
            <textarea disabled className={styles.textareas} 
                value='할인쿠폰 및 혜택, 이벤트, 신상품 소식 등 쇼핑몰에서 제공하는 유익한 쇼핑정보를 SMS나 이메일로 받아보실 수 있습니다.

                단, 주문/거래 정보 및 주요 정책과 관련된 내용은 수신동의 여부와 관계없이 발송됩니다.

                선택 약관에 동의하지 않으셔도 회원가입은 가능하며, 회원가입 후 회원정보수정 페이지에서 언제든지 수신여부를 변경하실 수 있습니다.
            '/>
            <p style={{fontSize:'11px'}}>SMS 수신을 동의하십니까?
            <input type='checkbox' name='third' value={joininput.third} onChange={onJoinChange} checked={joininput.third === true ? true : false}/></p>
            <p style={{fontSize:'11px'}}>이메일 수신을 동의하십니까? 
            <input type='checkbox' name='four' value={joininput.four} onChange={onJoinChange} checked={joininput.four === true ? true : false}/></p>
        </div>
    )
...
export default React.memo(Joininfo);